...
// Variables can be defined inline as well.
const ref = getArtworkByIdRef({ id: ..., });
...
## UpdateListingPrice
You can execute the `UpdateListingPrice` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
updateListingPrice(vars: UpdateListingPriceVariables): MutationPromise<UpdateListingPriceData, UpdateListingPriceVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
updateListingPrice(dc: DataConnect, vars: UpdateListingPriceVariables): MutationPromise<UpdateListingPriceData, UpdateListingPriceVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the updateListingPriceRef:
```typescript
const name = updateListingPriceRef.operationName;
console.log(name);
```

### Variables
The `UpdateListingPrice` mutation requires an argument of type `UpdateListingPriceVariables`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
...
```
### Return Type
Recall that executing the `UpdateListingPrice` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `UpdateListingPriceData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
export interface UpdateListingPriceData {
  listing_update?: Listing_Key | null;
}
```
### Using `UpdateListingPrice`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, updateListingPrice, UpdateListingPriceVariables } from '@dataconnect/generated';

// The `UpdateListingPrice` mutation requires an argument of type `UpdateListingPriceVariables`:
const updateListingPriceVars: UpdateListingPriceVariables = {
  id: ..., 
  currentPrice: ..., 
};

// Call the `updateListingPrice()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await updateListingPrice(updateListingPriceVars);
// Variables can be defined inline as well.
const { data } = await updateListingPrice({ id: ..., currentPrice: ..., });
...
// Or, you can use the `Promise` API.
updateListingPrice(updateListingPriceVars).then((response) => {
  const data = response.data;
  console.log(data.listing_update);
});
```

### Using `UpdateListingPrice`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, updateListingPriceRef, UpdateListingPriceVariables } from '@dataconnect/generated';
...
// Call the `updateListingPriceRef()` function to get a reference to the mutation.
const ref = updateListingPriceRef(updateListingPriceVars);
// Variables can be defined inline as well.
const ref = updateListingPriceRef({ id: ..., currentPrice: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = updateListingPriceRef(dataConnect, updateListingPriceVars);
